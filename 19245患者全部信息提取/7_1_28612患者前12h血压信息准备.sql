-- Materialized View: mimiciii.new_0709_ready_vitalsfirstday1

-- DROP MATERIALIZED VIEW mimiciii.new_0709_ready_vitalsfirstday1;

-- Description 

--   该程序功能为：计算患者进入ICU前12小时无创血压时序数据的衍生变量
--   程序流程为：选择符合要求的患者血压数据，并求解其最大值，最小值，平均值，方差，标准差，四分位数，极差。

--   程序运行结果；生成一个包含患者进入ICU前12小时的血压数据的衍生变量的表




CREATE MATERIALIZED VIEW mimiciii.new_0709_ready_vitalsfirstday1 AS 
 SELECT pvt.subject_id,      -- 计算最大值，最小值，平均值，方差，标准差，四分位数，极差等
   ...
   FROM ( SELECT ccc.subject_id,
            ...
            ccc.icustay_id,
                CASE
                    WHEN ccc.nisysbp::double precision > 0::double precision AND ccc.nisysbp::double precision < 400::double precision THEN ccc.nisysbp     		-- 选择符合设定正常值范围的数据
                    ELSE NULL::numeric
                END AS nisysbp,
                CASE
                ...
                CASE
                    ...
                    ELSE NULL::numeric
                END AS nimeanbp
           FROM mimiciii.rf0709_q12_runin ccc) pvt
  GROUP BY pvt.subject_id, pvt.hadm_id, pvt.icustay_id
  ORDER BY pvt.subject_id, pvt.hadm_id, pvt.icustay_id
WITH DATA;

ALTER TABLE mimiciii.new_0709_ready_vitalsfirstday1
  OWNER TO postgres;
