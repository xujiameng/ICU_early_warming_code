-- Materialized View: mimiciii.new_ready_runin

-- DROP MATERIALIZED VIEW mimiciii.new_ready_runin;


-- Description
--   该程序功能为：提取患者进入ICU第一天无创血压数据及有创血压数据
--   程序流程：根据官网提供的代码获取血压数据，并对血压值的异常数据进行处理

--   程序结果：生成患者进入ICU第一天的血压数据表




CREATE MATERIALIZED VIEW mimiciii.new_ready_runin AS 
 WITH chart_value AS (
         SELECT ie.subject_id,
           ...
           FROM mimiciii.icustays ie
             LEFT JOIN mimiciii.chartevents le ON le.subject_id = ie.subject_id AND le.hadm_id = ie.hadm_id AND ie.icustay_id = le.icustay_id AND le.charttime >= ie.intime AND le.charttime <= ie.outtime AND le.charttime >= ie.intime AND le.charttime <= (ie.intime + '1 day'::interval day) AND le.error IS DISTINCT FROM 1 AND (le.itemid = ANY (ARRAY[51, 6701, 220050, 8368, 8555, 220051, 52, 6702, 220052, 225312, 442, 455, 220179, 8440, 8441, 220180, 456, 443, 220181]))
        )     --提取患者进入ICU第一天信息的条件
 SELECT pvt.subject_id,
    ...
    pvt.charttime,
    max(
        CASE		--此处获取最大值是为了避免出现同一患者，同一时间出现多个数据的异常情况
...
ALTER TABLE mimiciii.new_ready_runin
  OWNER TO postgres;
