-- Materialized View: mimiciii.new_vitalsfirstday1

-- DROP MATERIALIZED VIEW mimiciii.new_vitalsfirstday1;


-- Description 

--   该程序功能为：计算患者进入ICU前12小时除无创血压外其它时序数据的衍生变量
--   程序流程为：选择符合要求的患者时序数据，并求解其最大值，最小值，平均值，方差，标准差，四分位数，极差。

--   程序运行结果；生成一个包含患者进入ICU前12小时的时序的衍生变量的表





CREATE MATERIALIZED VIEW mimiciii.new_vitalsfirstday1 AS 
   ...
   FROM ( SELECT ie.subject_id,
            ...
            ie.icustay_id,
                ...
                CASE
                    ...
                    ELSE ce.valuenum
                END AS valuenum
           FROM mimiciii.icustays ie   -- 选择仅是进入ICU前12小时的相关数据
             LEFT JOIN mimiciii.chartevents ce ON ie.subject_id = ce.subject_id AND ie.hadm_id = ce.hadm_id AND ie.icustay_id = ce.icustay_id AND ce.charttime >= ie.intime AND ce.charttime <= (ie.intime + '12:00:00'::interval hour) AND ce.error IS DISTINCT FROM 1
...
ALTER TABLE mimiciii.new_vitalsfirstday1
  OWNER TO postgres;
